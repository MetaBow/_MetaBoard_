...
static int txProcess(shtp_t *pShtp, uint8_t chan, const uint8_t* pData, uint32_t len)
{
    ...
    while (remaining > 0) {
        ...
        status = pShtp->pHal->write(pShtp->pHal, pShtp->outTransfer, lenField, pShtp->dev);
        while (status == 0)
        {
            shtp_service(pShtp);
            status = pShtp->pHal->write(pShtp->pHal, pShtp->outTransfer, lenField, pShtp->dev);
...
void *shtp_open(sh2_Hal_t *pHal, const struct device *dev)
{
    if (!shtp_initialized) {
...
void shtp_close(void *pInstance)
{
    ...
    pShtp->pHal->close(pShtp->pHal,pShtp->dev);
...
void shtp_service(void *pInstance)
{
    ...
    int len = pShtp->pHal->read(pShtp->pHal, pShtp->inTransfer, sizeof(pShtp->inTransfer), &t_us, pShtp->dev);
    if (len > 0) {
