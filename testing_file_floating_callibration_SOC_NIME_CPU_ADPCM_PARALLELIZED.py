...
class IMUAxisCalibrator:
    ...
    def process_quaternion(self, raw_quat: List[float]) -> List[float]:
        ...
        try:
            ...
            # Guard: skip calibration on invalid/zero-norm quaternions.
            # These happen during startup / sensor glitches and can hard-error in scipy,
            # which then cascades into lag/stalls.
            ...
            scipy_quat = [float(q[0]), float(q[1]), float(q[2]), float(q[3])]  # [x, y, z, w]
...
class OSCRouteManager:
    ...
    def get_available_routes(self):
        ...
        # Avoid printing on every UI refresh; this method is called very frequently and
        # console I/O can dominate runtime and make discovery/GUI feel stalled.
        return routes
...
class Window:
    ...
    def decode_data(self, data):
        """
        Decode incoming BLE data with ADPCM state header (NEW FIRMWARE FORMAT).

        Format:
            [0..2]    3 B   ADPCM state header:
                        [0..1] int16_t predicted_sample (little endian)
                        [2]    uint8_t step_index
            [3..47]   45 B   ADPCM audio data
            [48..111] 64 B   IMU bundle = 16 float32 (little endian):
                        0–3   quaternion (I, J, K, R)
                        4–6   linear accel (x, y, z)
                        7–9   gyroscope (x, y, z)
                        10–12 magnetometer (x, y, z)
                        13–15 raw accel (x, y, z)
            [112]     1 B   IMU present flag
            [113..116]4 B   battery SoC as float

        Returns:
            {
                'adpcm_header': dict,          # {'predicted_sample': int16, 'step_index': uint8}
                'adpcm_data': bytes,           # raw ADPCM block (45 bytes)
        ...
        """
        try:
            expected_packet_size = 117  # 3 header + 45 ADPCM + 64 IMU + 1 flag + 4 battery

            # NEW FIRMWARE: Packets are now 117 bytes (3-byte ADPCM state header added)
            # Old firmware sent 114 bytes, new firmware sends 117 bytes
            expected_packet_size_new = 117  # 3 header + 45 ADPCM + 64 IMU + 1 flag + 4 battery
            expected_packet_size_old = 114  # 45 ADPCM + 64 IMU + 1 flag + 4 battery (no header)
            
            # Validate packet size (allow both for transition period)
            if len(data) not in (expected_packet_size_old, expected_packet_size_new):
                self.log_message(
                    f"Invalid packet size: got {len(data)}, expected {expected_packet_size_old} (old) or {expected_packet_size_new} (new with header)",
                    "WARNING"
                )
                return None
            
            # Check if this is the new format (117 bytes) or old format (114 bytes)
            has_state_header = (len(data) == expected_packet_size_new)
            
            if has_state_header:
                # NEW FORMAT: Extract ADPCM state header
                predicted_sample = struct.unpack('<h', data[0:2])[0]  # int16 little endian
                step_index = data[2]  # uint8
                adpcm_header = {
                    'predicted_sample': predicted_sample,
                    'step_index': step_index
                }
                # ADPCM data starts at byte 3
                adpcm_data = data[3:48]  # 45 bytes
                # IMU starts at byte 48
                imu_start = 48
                imu_flag_index = 112
                battery_start_index = 113
                battery_end_index = 117
            else:
                # OLD FORMAT (backwards compatibility): No header
                adpcm_header = None
                adpcm_data = data[0:45]  # 45 bytes
                # IMU starts at byte 45
                imu_start = 45
                imu_flag_index = 109
            ...
            # 3) IMU data (16 floats)
            imu_flag = data[imu_flag_index]
            if imu_flag == 1:
                try:
                    imu_bytes = data[imu_start:imu_start+64]  # 64 bytes = 16 * 4
                    if len(imu_bytes) != 64:
                        self.log_message(
            ...
            return {
                'adpcm_header': adpcm_header,     # NEW: state header dict or None
                'adpcm_data': adpcm_data,        # raw ADPCM for decoding (45 bytes)
    ...
    def decode_data_with_route_detection(self, data) -> dict:
        ...
        # NEW FIRMWARE: Re-sync decoder state from header before decoding each block
        # This makes decoding robust to packet drops (each packet is self-contained)
        adpcm_header = decoded_data.get('adpcm_header')
        if adpcm_bytes and hasattr(self, "adpcm_decoder") and self.adpcm_decoder:
            try:
                ...
                # NEW: If header is present, re-sync decoder state from firmware
                # This allows recovery from packet drops (each packet is independent)
                if adpcm_header:
                    self.adpcm_decoder.set_state(
                        adpcm_header['predicted_sample'],
                        adpcm_header['step_index']
                    )
                # OLD FORMAT: If no header, decoder state persists (backwards compatibility)
                # (Decoder state will continue from previous packet)
                
                # Decode ADPCM (state is now synced from header if available)
...
class AudioRecorder:
    ...
    def start_recording(self, directory=None):
        ...
        # IMPORTANT:
        # Do NOT reset the ADPCM decoder here.
        # Firmware resets the encoder on BLE connect; resetting the decoder mid-stream
        # guarantees encoder/decoder state mismatch and produces noise in the WAV.
