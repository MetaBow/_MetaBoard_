...
class RealTimeAudioFeatureExtractor:
    ...
    def __init__(
        ...
        frame_size=2048,
        hop_length=512,
        frame_callback: Optional[Callable[[int, Dict[str, Any], float, float], None]] = None,
        mono_anchor_t0: Optional[float] = None,
    ):
        ...
        self.mono_anchor_t0 = mono_anchor_t0
    ...
    def start_processing_thread(self):
        ...
        if self.mono_anchor_t0 is None:
            self.mono_anchor_t0 = time.perf_counter()
            ...
            try:
                if len(self.audio_buffer) >= self.frame_size:
                    ...
                    if self.frame_callback:
                        mono_sec = time.perf_counter() - float(self.mono_anchor_t0)
                        try:
                            self.frame_callback(
                                self.features_processed,
                                self.get_all_feature_values(),
                                time.time(),
                                mono_sec,
                            )
                        except Exception as cb_exc:  # noqa: BLE001
                            print(f"frame_callback error: {cb_exc}")
