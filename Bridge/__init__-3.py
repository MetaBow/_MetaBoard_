...
class FlowControlledConnection:
    ...
    def setup_osc(self):
        """Setup OSC with error handling for dual ports"""
        if OSC_AVAILABLE:
            try:
                # Create clients for both ports
                self.osc_8888 = udp_client.SimpleUDPClient("127.0.0.1", 8888)
                self.osc_8889 = udp_client.SimpleUDPClient("127.0.0.1", 8889)
                self.log_callback("OSC clients connected to ports 8888 and 8889")
            except Exception as e:
                ...
                try:
                    ...
                    if motion_start >= 0 and motion_start + imu_data_len <= data_len - flag_size:
                        ...
                        if len(motion_data) == imu_data_len:
                            ...
                            if self.osc_8888:
                                try:
                                    self.osc_8888.send_message("/motion", motion_floats)
                                except:
                                    pass  # Silently ignore OSC errors
                            
                            if self.osc_8889:
                                try:
                                    self.osc_8889.send_message("/motion", motion_floats)
                                except:
                                    pass  # Silently ignore OSC errors
...
class AntiStallApp:
    ...
    def create_ui(self):
        ...
        if OSC_AVAILABLE:
            self.log("OSC enabled - motion data will be sent to ports 8888 and 8889")
