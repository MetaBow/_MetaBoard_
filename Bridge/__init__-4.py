...
- Fixed gentle reset monitoring issue
- Added dual port OSC support (8888 and 8889)
"""
...
class BLEConnection:
    ...
    def _process_packet(self, data, timestamp):
        ...
        try:
            ...
            # Motion data (IMU) - Send to both OSC ports
                ...
                try:
                    ...
                    if motion_start >= 0:
                        ...
                        if len(motion_data) == imu_data_len:
                            ...
                            # Send to all configured OSC ports
                            for port_name, osc_client in self.osc_clients:
                                try:
                                    osc_client.send_message("/motion", motion_floats)
                                except Exception as e:
                                    # Log OSC errors but don't stop processing
                                    if self.message_count % 5000 == 0:  # Throttle error logging
                                        self.log(f"OSC error on port {port_name}: {e}")
                except:
...
class MetaBowApp:
    ...
    def create_ui(self):
        ...
        # OSC status - Updated to show both ports
        if OSC_AVAILABLE:
            osc_text = "OSC: Available (ports 8888 & 8889)"
            osc_color = "green"
        else:
        ...
        if OSC_AVAILABLE:
            self.log("OSC enabled - motion data sent to ports 8888 & 8889")
    ...
    def start_monitoring(self):
        """Start connection monitoring with improved reset handling"""
        self.stop_monitoring()  # Stop any existing monitor
        self.monitor_active = True
        
        def monitor():
            try:
                while self.monitor_active and self.is_connected and not self._shutting_down:
                    # Wait if reset is in progress
                    if self.reset_in_progress:
                        time.sleep(0.2)
                        continue
                    
                    # Check BLE client connection
                    ...
                    if self.connection:
                        ...
                        if (current_threshold and 
                            ...
                            # Perform reset and wait for completion
                            reset_success = self.perform_monitored_reset()
                            if not reset_success:
                                break  # Connection lost during reset
                            
                            continue  # Continue monitoring after successful reset
                ...
                # Connection lost or monitoring stopped
                if (self.is_connected and not self._shutting_down and 
                    not self.reset_in_progress and self.monitor_active):
                    self.root.after(0, self.handle_disconnect)
            
            except Exception as e:
                print(f"Monitor error: {e}")
                # Only restart monitoring if we're still supposed to be connected
                if (self.is_connected and not self._shutting_down and 
                    self.monitor_active and not self.reset_in_progress):
                    time.sleep(1)
                    self.start_monitoring()
    ...
    def perform_monitored_reset(self):
        """Perform reset with proper synchronization"""
        if not self.is_connected or not self.connection or self.reset_in_progress:
            return False
        ...
        try:
            ...
            if result == "Success":
                indicator = "🔴 " if self.is_sequoia else ""
                self.root.after(0, self.log, f"{indicator}Monitored reset successful")
                self.last_reset_count = 0
                success = True
            else:
                self.root.after(0, self.log, f"Monitored reset failed: {result}")
                success = False
            ...
            self.root.after(0, self.log, f"Monitored reset error: {e}")
            success = False
    ...
    def gentle_reset(self):
        ...
        if self.reset_in_progress:
            self.log("Reset already in progress")
            return
        
        self.reset_in_progress = True
        
        indicator = "🔴 " if self.is_sequoia else ""
        self.log(f"{indicator}Performing manual gentle reset...")
    ...
    def handle_reset_result(self, result):
        ...
        if result == "Success":
            self.log(f"{indicator}Manual gentle reset successful!")
            self.last_reset_count = 0
        else:
            self.log(f"{indicator}Manual gentle reset failed: {result}")
            # Check connection status after failed reset
            if self.client and not self.client.is_connected:
    ...
    def emergency_reset(self):
        ...
        # Stop monitoring during emergency reset
        self.stop_monitoring()
...
if __name__ == "__main__":
    ...
    print("- Dual-port OSC output (8888 & 8889)")
